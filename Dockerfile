...
COPY /plan/app-config/ ${PLAN_DIR}/app-config/
...
RUN cd ${PLAN_DIR}/celestia-app \
    && go build -o ${PLAN_DIR}/build/ ./cmd/celestia-appd
...
COPY --from=builder ${PLAN_DIR}/build/celestia-appd /celestia-appd

COPY --from=builder ${PLAN_DIR}/app-config /appconfig
...
EXPOSE 6060 26656 26657 1317 9090
ENTRYPOINT [ "/testplan"]